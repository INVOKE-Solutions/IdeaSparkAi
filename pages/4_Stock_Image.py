...
if check_password():
    ...
    1. Enter a prompt in the text box to generate a stock image.
    2. Select the size of the image from the options: 256x256px, 512x512px, or 1024x1024px.
    3. Click the "Generate Stock Images" button to generate the image.
    4. Wait for the image to be processed and generated.
    5. Scroll down to view the generated stock image.
    """)
    st.info(""" NOTE: You can download image by right clicking on the 
            image and select save image as option""")  
    ...
    with st.form(key="translation_form"):
                ...
                prompt_input = st.text_input(label="**Enter image prompt**")

                # Select box to select the size of the images
                size = st.selectbox(label="**Select image size**", options=["1024x1024", "1024x1792", "1792x1024"])

                # Submit button
                submit_button = st.form_submit_button(label="**Generate Stock Images**")
    ...
    if submit_button:
                ...
                if not prompt_input:
                    ...
                    if lang == 'ms':
                        ...
                        stock_prompt = f"Generate stock images of {prompt} "
                    else:
